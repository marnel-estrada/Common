// Copied from https://docs.unity3d.com/6000.3/Documentation/Manual/DefaultPresetsByFolder.html
...
namespace Common {
    ...
    ///     The code is divided into three parts that set up importer dependencies to make sure the importers of all Assets
    ///     stay deterministic.
    ///     OnPreprocessAsset:
    ...
    ///     If it is applied, the method adds a direct dependency to each Preset so that the Asset can be re-imported when the
    ///     Preset values are changed.
    /// </summary>
    public class EnforcePresetPostProcessor : AssetPostprocessor {
        /// <summary>
        ///     This method with the didDomainReload argument will be called every time the project is being loaded or the code is
        ///     compiled.
        ...
        private void OnPreprocessAsset() {
            ...
            if (this.assetPath.StartsWith("Assets/") && !AssetDatabase.IsValidFolder(this.assetPath) &&
                !this.assetPath.EndsWith(".cs") && !this.assetPath.EndsWith(".preset")) {
                string? path = Path.GetDirectoryName(this.assetPath);
        ...
        private void ApplyPresetsFromFolderRecursively(string folder) {
            ...
            this.context.DependsOnCustomDependency($"PresetPostProcessor_{folder}");
    ...
    ///     For each folder containing a Preset, create a CustomDependency from the folder name and a hash from the list of
    ///     Preset names and types in the folder.
    ///     OnAssetsModified:
    ///     Whenever a Preset is added, removed, or moved from a folder, the CustomDependency for this folder needs to be
    ///     updated
    ...
    ///     This makes the InitPresetDependencies and OnPostprocessAllAssets methods too complex for the purpose of this
    ///     example.
