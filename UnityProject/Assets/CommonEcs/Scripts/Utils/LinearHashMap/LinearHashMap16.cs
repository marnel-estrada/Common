...
namespace CommonEcs {
    public struct LinearHashMap16<K, V>
        ...
        private readonly int LinearProbeForAdding(int hashCode, int startingIndex) {
            const int maxCount = LinearHashMapBucket16<K, V>.Length;
        ...
        private static int ComputeBucketIndex(int hash) {
            ...
            return hash & (LinearHashMapBucket16<K, V>.Length - 1);
        }

        public int Count => this.count;

        public void Clear() {
            for (int i = 0; i < LinearHashMapBucket16<K, V>.Length; ++i) {
        ...
        public LinearHashMapBucket16<K, V>.Enumerator Entries => this.bucket.GetEnumerator();
    }
}