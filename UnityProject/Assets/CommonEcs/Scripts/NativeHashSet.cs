...
namespace CommonEcs {
    ...
    public unsafe struct NativeHashSet<T> : IDisposable
        ...
        public NativeHashSet(int capacity, Allocator allocator) {
            ...
            // Allocate the state
            NativeHashSetState* state = (NativeHashSetState*) UnsafeUtility.Malloc(sizeof(NativeHashSetState),
                UnsafeUtility.AlignOf<NativeHashSetState>(), allocator);
            ...
            int nextOffset;
            int bucketOffset;
            int totalSize = CalculateDataLayout(capacity, bucketLength, out nextOffset, out bucketOffset);
        ...
        public int Length {
            get {
                ...
                NativeHashSetState* state = this.m_State;
                int* nextPtrs = (int*) state->Next;
                int freeListSize = 0;
                for (int tls = 0; tls < JobsUtility.MaxJobThreadCount; ++tls) {
                    for (int freeIdx = state->FirstFreeTLS[tls * NativeHashSetState.IntsPerCacheLine]; freeIdx >= 0;
        ...
        public int Capacity {
            get {
                ...
                return this.m_State->ItemCapacity;
            }
            set {
                RequireWriteAccess();
        ...
        public bool TryAdd(T item) {
            ...
            if (TryGetFirstValueAtomic(this.m_State, item, out tempIt)) {
                return false;
            }
            ...
            if (this.m_State->AllocatedIndexLength >= this.m_State->ItemCapacity && this.m_State->FirstFreeTLS[0] < 0) {
                for (int tls = 1; tls < JobsUtility.MaxJobThreadCount; ++tls) {
                    if (this.m_State->FirstFreeTLS[tls * NativeHashSetState.IntsPerCacheLine] >= 0) {
                        idx = this.m_State->FirstFreeTLS[tls * NativeHashSetState.IntsPerCacheLine];
                        nextPtrs = (int*) this.m_State->Next;
                        this.m_State->FirstFreeTLS[tls * NativeHashSetState.IntsPerCacheLine] = nextPtrs[idx];
                        nextPtrs[idx] = -1;
                        this.m_State->FirstFreeTLS[0] = idx;

                        break;
                    }
                }

                if (this.m_State->FirstFreeTLS[0] < 0) {
                    int capacity = this.m_State->ItemCapacity;
            ...
            idx = this.m_State->FirstFreeTLS[0];
            if (idx >= 0) {
                this.m_State->FirstFreeTLS[0] = ((int*) this.m_State->Next)[idx];
            } else {
                idx = this.m_State->AllocatedIndexLength++;
            }

            // Write the new value to the entry
            UnsafeUtility.WriteArrayElement(this.m_State->Items, idx, item);

            int bucket = item.GetHashCode() & this.m_State->BucketCapacityMask;
            // Add the index to the hash-set
            int* buckets = (int*) this.m_State->Buckets;
            nextPtrs = (int*) this.m_State->Next;
        ...
        public void Clear() {
            ...
            int* buckets = (int*) this.m_State->Buckets;
            for (int i = 0; i <= this.m_State->BucketCapacityMask; ++i) {
                buckets[i] = -1;
            }

            int* nextPtrs = (int*) this.m_State->Next;
            for (int i = 0; i < this.m_State->ItemCapacity; ++i) {
                nextPtrs[i] = -1;
            }

            for (int tls = 0; tls < JobsUtility.MaxJobThreadCount; ++tls) {
                this.m_State->FirstFreeTLS[tls * NativeHashSetState.IntsPerCacheLine] = -1;
            }

            this.m_State->AllocatedIndexLength = 0;
        }
        ...
        public bool Remove(T item) {
            ...
            while (entryIdx >= 0 && entryIdx < this.m_State->ItemCapacity) {
                if (UnsafeUtility.ReadArrayElement<T>(this.m_State->Items, entryIdx).Equals(item)) {
                    ...
                    // And free the index
                    nextPtrs[entryIdx] = this.m_State->FirstFreeTLS[0];
                    this.m_State->FirstFreeTLS[0] = entryIdx;
        ...
        public bool Contains(T item) {
            ...
            return TryGetFirstValueAtomic(this.m_State, item, out tempIt);
        }
        ...
        public NativeArray<T> ToNativeArray(NativeArray<T> array = default, int index = 0) {
            ...
            if (!array.IsCreated || array.Length - index < length) {
                array = new NativeArray<T>(length + index, this.m_Allocator);
            }

            int* bucketArray = (int*) this.m_State->Buckets;
            int* bucketNext = (int*) this.m_State->Next;
            for (int i = 0; i <= this.m_State->BucketCapacityMask; ++i) {
                for (int b = bucketArray[i]; b != -1; b = bucketNext[b]) {
                    array[index] = UnsafeUtility.ReadArrayElement<T>(this.m_State->Items, b);
                    index++;
                }
            }
        ...
        private void Deallocate() {
            UnsafeUtility.Free(this.m_State->Items, this.m_Allocator);
            this.m_State->Items = null;
            this.m_State->Next = null;
            this.m_State->Buckets = null;
            UnsafeUtility.Free(this.m_State, this.m_Allocator);
            this.m_State = null;
        }
        ...
        private static int AllocFreeListEntry(NativeHashSetState* state, int threadIndex) {
            ...
            do {
                ...
                if (idx < 0) {
                    ...
                    Interlocked.Exchange(ref state->FirstFreeTLS[threadIndex * NativeHashSetState.IntsPerCacheLine],
                        -2);
                    ...
                    if (state->AllocatedIndexLength < state->ItemCapacity) {
                        ...
                        if (idx < state->ItemCapacity - 1) {
                            ...
                            Interlocked.Exchange(
                                ref state->FirstFreeTLS[threadIndex * NativeHashSetState.IntsPerCacheLine], idx + 1);
                        ...
                        if (idx == state->ItemCapacity - 1) {
                            Interlocked.Exchange(
                                ref state->FirstFreeTLS[threadIndex * NativeHashSetState.IntsPerCacheLine], -1);
                    ...
                    while (again) {
                        again = false;
                        for (int other = (threadIndex + 1) % JobsUtility.MaxJobThreadCount; other != threadIndex;
                            ...
                            do {
                                ...
                                ref state->FirstFreeTLS[other * NativeHashSetState.IntsPerCacheLine], nextPtrs[idx],
                                idx) != idx);
                ...
                ref state->FirstFreeTLS[threadIndex * NativeHashSetState.IntsPerCacheLine], nextPtrs[idx], idx) != idx);
        ...
        private static bool TryGetFirstValueAtomic(NativeHashSetState* state, T item,
        ...
        public struct ParallelWriter {
            ...
            public int Capacity {
                get {
                    this.m_Set.RequireReadAccess();

                    return this.m_Set.m_State->ItemCapacity;
                }
            }
            ...
            /// </returns>
            public bool TryAdd(T item) {
                this.m_Set.RequireWriteAccess();

                NativeMultiHashSetIterator tempIt;
                if (TryGetFirstValueAtomic(this.m_Set.m_State, item, out tempIt)) {
                    return false;
                }

                // Allocate an entry from the free list
                int idx = AllocFreeListEntry(this.m_Set.m_State, this.m_ThreadIndex);

                // Write the new value to the entry
                UnsafeUtility.WriteArrayElement(this.m_Set.m_State->Items, idx, item);

                int bucket = item.GetHashCode() & this.m_Set.m_State->BucketCapacityMask;
                // Add the index to the hash-set
                int* buckets = (int*) this.m_Set.m_State->Buckets;
                if (Interlocked.CompareExchange(ref buckets[bucket], idx, -1) != -1) {
                    int* nextPtrs = (int*) this.m_Set.m_State->Next;
                    do {
                        nextPtrs[idx] = buckets[bucket];
                        if (TryGetFirstValueAtomic(this.m_Set.m_State, item, out tempIt)) {
                            ...
                            do {
                                nextPtrs[idx] =
                                    this.m_Set.m_State->FirstFreeTLS[
                                        this.m_ThreadIndex * NativeHashSetState.IntsPerCacheLine];
                            } while (Interlocked.CompareExchange(
                                    ref this.m_Set.m_State->FirstFreeTLS[
                                        this.m_ThreadIndex * NativeHashSetState.IntsPerCacheLine], idx,
                                    nextPtrs[idx]) !=
                                nextPtrs[idx]);
...
#if ENABLE_UNITY_COLLECTIONS_CHECKS
                AtomicSafetyHandle.SetAllowReadOrWriteAccess(this.m_Set.m_Safety, allowReadOrWriteAccess);
#endif
            }
        }
        ...
        private void Reallocate(int newCapacity) {
            ...
            if (this.m_State->ItemCapacity == newCapacity &&
                this.m_State->BucketCapacityMask + 1 == newBucketCapacity) {
                return;
            }

            // Can't shrink
            if (newCapacity < this.m_State->ItemCapacity) {
            ...
            int nextOffset;
            int bucketOffset;
            int totalSize = CalculateDataLayout(newCapacity, newBucketCapacity, out nextOffset, out bucketOffset);
            byte* newData = (byte*) UnsafeUtility.Malloc(totalSize, JobsUtility.CacheLineSize, this.m_Allocator);
            ...
            UnsafeUtility.MemCpy(newItems, this.m_State->Items, this.m_State->ItemCapacity * UnsafeUtility.SizeOf<T>());
            UnsafeUtility.MemCpy(newNext, this.m_State->Next, this.m_State->ItemCapacity * UnsafeUtility.SizeOf<int>());
            for (int emptyNext = this.m_State->ItemCapacity; emptyNext < newCapacity; ++emptyNext) {
            ...
            for (int bucket = 0; bucket <= this.m_State->BucketCapacityMask; ++bucket) {
                int* buckets = (int*) this.m_State->Buckets;
                int* nextPtrs = (int*) newNext;
                while (buckets[bucket] >= 0) {
                    int curEntry = buckets[bucket];
                    buckets[bucket] = nextPtrs[curEntry];
                    int newBucket = UnsafeUtility.ReadArrayElement<T>(this.m_State->Items, curEntry).GetHashCode() &
                        (newBucketCapacity - 1);
            ...
            if (this.m_State->AllocatedIndexLength > this.m_State->ItemCapacity) {
                this.m_State->AllocatedIndexLength = this.m_State->ItemCapacity;
            }

            this.m_State->Items = newData;
            this.m_State->Next = newNext;
            this.m_State->Buckets = newBuckets;
            this.m_State->ItemCapacity = newCapacity;
            this.m_State->BucketCapacityMask = newBucketCapacity - 1;
        }
        ...
        public void TestUseOnlySetAllowReadAndWriteAccess(bool allowReadOrWriteAccess) {
#if ENABLE_UNITY_COLLECTIONS_CHECKS
            AtomicSafetyHandle.SetAllowReadOrWriteAccess(this.m_Safety, allowReadOrWriteAccess);
#endif
        }
    }
    ...
    internal sealed class NativeHashSetDebugView<T>
        ...
        public List<T> Items {
            get {
                using (NativeArray<T> array = this.m_Set.ToNativeArray()) {
                    List<T> result = new List<T>(array.Length);
                    foreach (T item in array) {
                        if (this.m_Set.Contains(item)) {
                            result.Add(item);
                        }
                    }
