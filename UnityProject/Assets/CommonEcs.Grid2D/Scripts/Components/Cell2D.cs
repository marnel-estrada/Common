...
namespace CommonEcs {
    ...
    public readonly struct Cell2D : IComponentData {
        // Note that this is 3D since the z value refers to the floor level
        public readonly int3 gridCoordinate; // This is the coordinate where (0, 0) is at the bottom left of the grid
        ...
        public Cell2D(Entity grid, int3 gridCoordinate, int2 worldCoordinate, float2 size, int index, float2 center) {
            this.grid = grid;
