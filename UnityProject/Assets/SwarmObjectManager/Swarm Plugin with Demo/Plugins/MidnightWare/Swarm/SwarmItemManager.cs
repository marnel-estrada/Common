...
public class SwarmItemManager : MonoBehaviour
{
	...
	protected class PrefabItemLists
	{
		...
		public int ItemCount {
			get {
				return activeItems.Count + inactiveItems.Count;
			}
		}
	...
	public virtual void Initialize()
	{
		...
		{
			Debug.Log("WARNING! No Item Prefabs exists for " + gameObject.name + " -- Errors will occur.", this.gameObject);
		}
	...
	public virtual SwarmItem ActivateItem(int itemPrefabIndex, bool forceInstantiate = false)
	{
		...
		while (!forceInstantiate && localItem == null && _prefabItemLists[itemPrefabIndex].inactiveItems.Count > 0) {
			localItem = _prefabItemLists[itemPrefabIndex].inactiveItems.Pop();
		}
		...
		if(localItem == null || forceInstantiate) {
			...
			_prefabItemLists[itemPrefabIndex].activeItems.AddLast(localItem);
		...
		SetItemParentTransform(localItem, _activeParentTransform);
	...
	/// <summary>
	/// A convenience method to prune the inactive list with the specified pruneCount. This is a hack so it might get overridden when updates to the plugin is made.
	/// </summary>
	/// <param name='itemPrefabIndex'>
	/// Item prefab index.
	/// </param>
