...
namespace GoapBrainEcs {
    public class MultipleAtomActions : GoapExecutionTestTemplate {
        // Conditions
        ...
        public MultipleAtomActions(World world, EntityManager entityManager) : base(world, entityManager) {
        }
        ...
        protected override void DoAssertions(EntityManager entityManager) {
            ...
            // We only add one here because there's only one IncrementCounterComposer after IncrementCounterUntilComposer
            // IncrementCounterUntilComposer increases the counter until MAX_VALUE then the extra IncrementCounterComposer
            // adds 1.
            Assert.IsTrue(counter.value == MAX_VALUE + 1); 
        }
    }
}