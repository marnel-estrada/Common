...
namespace Common {
    ...
    ///
    /// Got it from https://stackoverflow.com/questions/452902/how-to-read-a-text-file-reversely-with-iterator-in-c-sharp
    /// </summary>
    public sealed class ReverseLineReader : IEnumerable<string> {
        ...
        public ReverseLineReader(string filename, Encoding encoding) : this(() => File.OpenRead(filename), encoding) {
        }
        ...
        public ReverseLineReader(Func<Stream> streamSource, Encoding encoding) : this(streamSource, encoding,
            DefaultBufferSize) {
        }
