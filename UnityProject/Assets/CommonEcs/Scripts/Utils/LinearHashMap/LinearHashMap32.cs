...
namespace CommonEcs {
    public struct LinearHashMap32<K, V>
        where K : unmanaged, IEquatable<K>
        where V : unmanaged {
        private LinearHashMapBucket32<K, V> bucket;
        private int count;
        ...
        private readonly int LinearProbeForAdding(int hashCode, int startingIndex) {
            const int maxCount = LinearHashMapBucket32<K, V>.Length;
        ...
        private static int ComputeBucketIndex(int hash) {
            ...
            return hash & (LinearHashMapBucket32<K, V>.Length - 1);
        }

        public int Count => this.count;

        public void Clear() {
            for (int i = 0; i < LinearHashMapBucket32<K, V>.Length; ++i) {
        ...
        public LinearHashMapBucket32<K, V>.Enumerator Entries => this.bucket.GetEnumerator();
    }
}