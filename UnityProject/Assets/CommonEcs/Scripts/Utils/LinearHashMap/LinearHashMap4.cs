...
namespace CommonEcs {
    public struct LinearHashMap4<K, V> 
        where K : unmanaged, IEquatable<K>
        where V : unmanaged {
        private LinearHashMapBucket4<K, V> bucket;
        private int count;
        ...
        private readonly int LinearProbeForAdding(int hashCode, int startingIndex) {
            const int maxCount = LinearHashMapBucket4<K, V>.Length;
        ...
        private static int ComputeBucketIndex(int hash) {
            ...
            return hash & (LinearHashMapBucket4<K, V>.Length - 1);
        }
        
        public int Count => this.count;

        public void Clear() {
            for (int i = 0; i < LinearHashMapBucket4<K, V>.Length; ++i) {
        ...
        public LinearHashMapBucket4<K, V>.Enumerator Entries => this.bucket.GetEnumerator();
    }
}