...
namespace CommonEcs.Goap {
    ...
    public struct ConditionsMap {
        ...
        private ConditionValueMap map;
        private DynamicBuffer<ConditionValueMap.Entry> bucket;
        ...
        public static void Init(ref EntityManager entityManager, in Entity plannerEntity) {
            DynamicBuffer<ConditionValueMap.Entry> bucket = 
                entityManager.GetBuffer<ConditionValueMap.Entry>(plannerEntity);
            ConditionValueMap.Init(ref bucket);
        }

        private ConditionsMap(in Entity plannerEntity, ConditionValueMap map, 
            DynamicBuffer<ConditionValueMap.Entry> bucket) {
        ...
        public ConditionsMap(ref EntityManager entityManager, in Entity plannerEntity) : this(plannerEntity, 
            entityManager.GetComponentData<ConditionValueMap>(plannerEntity),
            entityManager.GetBuffer<ConditionValueMap.Entry>(plannerEntity)) {
        }
        ...
        public static void ResetValues(ref DynamicBuffer<ConditionValueMap.Entry> bucket) {
            ConditionValueMap.ResetValues(ref bucket);
        }
    }
}